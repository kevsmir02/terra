...
# zsh, so we shadow $? into `_terra_ret`.

{
  _terra_user_zdotdir="${TERRA_USER_ZDOTDIR:-$HOME}"
  [ -f "$_terra_user_zdotdir/.zshrc" ] && source "$_terra_user_zdotdir/.zshrc"
...
if [[ -z "$__TERRA_HOOKS_LOADED" ]]; then
  ...
  _terra_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_terra_urlencode "$PWD")"
  ...
  _terra_preexec() {
    ...
    [[ -n "$TERRA_BLOCKS" ]] && _terra_block_seen=1
    local cmd="${1//[[:cntrl:]]/ }"
    printf '\e]133;C;%s\e\\' "${cmd[1,256]}"
  }

  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _terra_precmd
    add-zsh-hook preexec _terra_preexec
  fi
