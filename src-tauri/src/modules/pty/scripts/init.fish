...
# A/B (the `mark-prompt` feature); Terra disables it at spawn via
# fish_features=no-mark-prompt so these markers aren't emitted twice.

# Installed into conf.d, which every fish session sources; only Terra-spawned
# shells (TERRA_TERMINAL=1) may get their prompt wrapped.
if not set -q TERRA_TERMINAL
    exit 0
end
if set -q __TERRA_HOOKS_LOADED
    exit 0
end
set -g __TERRA_HOOKS_LOADED 1

# Terra is a clean terminal; drop fish's default startup greeting. A user who
# sets their own in config.fish (sourced after this) keeps it.
function fish_greeting
end

set -g __TERRA_HOST (uname -n 2>/dev/null; or echo localhost)

# URL-encode a path keeping `/` intact so it stays valid inside file://.
function __terra_urlencode_path
...
function __terra_restore_status
    return $argv[1]
end

function __terra_capture_user_prompt
    if not functions -q fish_prompt
        return
    end
    if functions fish_prompt | string match -q '*__terra_user_prompt*'
        return
    end
    functions -e __terra_user_prompt 2>/dev/null
    functions -c fish_prompt __terra_user_prompt
end

# Wrapped so `fish -C __terra_install_prompt` can re-run it AFTER config.fish,
