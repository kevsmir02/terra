...
Android device and emulator mirroring, driven by the platform-tools on PATH. `adb.rs` resolves `adb` / `emulator` / `avdmanager` and parses their output; `server.rs` pushes and starts the bundled `scrcpy-server-*.jar` (shipped via `bundle.resources`); `session.rs` owns a live mirror session (adb forward + video socket) and `remux.rs` consumes the server's frame-metadata packets, packaging one fMP4 fragment per access unit with capture timestamps for MSE playback in the webview; `control.rs` encodes touch/key/scroll control messages; `state.rs` holds sessions and Terra-launched AVDs, all killed on `RunEvent::Exit` (AVDs the user started elsewhere are left alone). Video frames cross the IPC boundary as raw bytes on a `Channel<Response>` with a leading discriminator byte (`remux::FRAME_INIT` / `FRAME_MEDIA`), the same raw-byte path the terminal uses. A second channel, `on_exit: Channel<DeviceExit>`, reports why a reader stopped (`stream-ended`, `server-unreachable`, `stream-corrupt`, `stream-error: <io>`) and stays silent when the webview asked for the teardown, so the pane can dim the frozen last frame and offer Reconnect; the mirror never reconnects on its own (`docs/adr/0001-mirror-does-not-reconnect-automatically.md`).
