...
The asset protocol (`asset://`, used for image/video/audio/PDF previews) has an **empty static scope**. `fs_allow_asset` grants one already-authorized file at a time via `asset_protocol_scope().allow_file()`. A blanket `"**"` scope would hand the webview arbitrary file reads over a channel this registry never sees. The PDF frame is sandboxed with `allow-scripts allow-same-origin` and nothing else (`editor/lib/pdfFrame.ts`, tested): WebKitGTK's bundled PDF.js viewer fetches the document with the frame's origin, so an opaque origin fails the asset protocol's CORS check and shows an empty toolbar, while withholding every navigation grant keeps a crafted PDF from moving the webview off the app.
...
- **markdown/**: markdown preview renderer (backs the `markdown` tab kind), lazy. Mermaid fences render as diagrams through Streamdown's diagram plugin, loaded in a second step: `lib/useMermaid.ts` imports `@streamdown/mermaid` (and with it `mermaid`, the largest dependency in the tree) only once a rendered document contains a fence (`lib/mermaidFence.ts`, pure and tested), and `eager-budget.test.ts` locks it out of both the eager graph and the markdown stack's static graph. Diagrams take the active theme: `lib/mermaidTheme.ts` maps the resolved theme tokens onto Mermaid's base theme variables, re-emitted as hex because Mermaid's colour parser does not know oklch.
