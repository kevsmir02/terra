...
- **header/**: top bar + inline search (`SearchInline` adapts to terminal vs editor via `SearchTarget`). `WindowControls` renders the custom minimize, maximize, and close buttons.
- **statusbar/**: bottom bar, `CwdBreadcrumb` (path segments and home `~` via `pathUtils.segmentsFromCwd`), the LSP pill, the diagnostics badge.
