...
- With no system image on disk the dock offers to install one. Terra never downloads it: `device_sdk_install_command` resolves a single `sdkmanager` line from a hardcoded catalog (last three API levels, `google_apis`, host ABI) and `runInTerminal` runs it in a terminal tab, so the multi-gigabyte download and Google's licence prompts happen in front of the user and `PtyState` owns the teardown (`docs/adr/0004-sdk-install-runs-in-a-terminal-tab.md`). That line is a shell *line*, not an argv, so `build_sdk_install_command` refuses any package outside the catalog and quotes every element; the test runs the result back through `sh` and compares the argv. Completion is the image appearing on disk: `useSdkSetup` polls `device_list_system_images` only while an install is in flight, and creates the AVD when it lands.
