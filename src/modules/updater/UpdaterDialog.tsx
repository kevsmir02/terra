...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    case "arch":
      return "yay -S terra-bin";
    case "debian":
      return `sudo apt install ./Terra_${version}_amd64.deb`;
    case "fedora":
      return `sudo dnf install ./Terra-${version}-1.x86_64.rpm`;
  }
}
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading update…"
                : manual
                  ? `Terra v${manual.version} is available`
                  : `Terra v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart Terra to finish installing."
              : downloading
                ? progress !== null
