...
function getRecycler(): HTMLDivElement {
  ...
  el.setAttribute("data-terra-recycler", "");
  el.style.cssText =
...
function createSlot(): Slot {
  ...
  host.setAttribute("data-terra-slot", String(slots.length));
  ...
  if (!fast) {
    ...
    if (p.snapshot) {
      try {
        ...
        console.warn("[terra] snapshot replay failed:", e);
      }
    }
    if (p.altScreen) {
...
function serializeSlot(slot: Slot): SerializeOutput {
  ...
  try {
    ...
    console.warn("[terra] serialize failed:", e);
  }
  return {
    snapshot,
    cols: slot.term.cols,
...
function disposeSlot(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[terra] slot dispose failed:", e);
  }
  slot.host.remove();
...
function attachWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[terra-webgl] unavailable:", e);
  }
}
...
function disposeSlotWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[terra-webgl] dispose failed:", e);
  }
  try {
    const r = (
