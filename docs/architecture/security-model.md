...
2. **File-system boundary** - PTY spawn goes through the workspace authorization registry.
3. **Network boundary** - HTTP proxy in `src-tauri/src/modules/net.rs` with SSRF and DNS-rebinding defenses.
4. **Terminal escape-sequence boundary** - OSC sequences are parsed and acted on, but never blindly trusted to mutate state.
...
`src-tauri/src/modules/net.rs` proxies outbound HTTP requests. Before connecting:
...
- **OSC 777** is used by the agent detector to signal coding-agent state transitions in the terminal.
